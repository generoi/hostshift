...
const (
	...
	variant     = "https://wt-a--acmecorp.ddev.site"
	variantHost = "wt-a--acmecorp.ddev.site"

	natCanonical   = "https://www.acmecorpnat.fi"
	natVariant     = "https://wt-a--nat.acmecorp.ddev.site"
	natVariantHost = "wt-a--nat.acmecorp.ddev.site"
)

// acmecorpMap is the two-blog map, with the ddev host listed as an alias so a
// residual @ddev URL is corrected too (PLAN §4.2).
func acmecorpMap(t *testing.T) *origin.Map {
	t.Helper()
	m, err := origin.NewMap([]origin.Site{
		{
			Name:      "main",
			Canonical: origin.MustParse(canonical),
			Aliases:   []origin.Origin{origin.MustParse("https://acmecorp.ddev.site")},
			Variant:   origin.MustParse(variant),
		},
		{
			Name:      "nat",
			Canonical: origin.MustParse(natCanonical),
			Aliases:   []origin.Origin{origin.MustParse("https://nat.acmecorp.ddev.site")},
...
func TestMultisiteInverseMapping(t *testing.T) {
	...
	if h.seen.Host != "www.acmecorpnat.fi" {
	...
	if h2.seen.Host != "www.acmecorp.fi" {
...
func TestCrossBlogLink(t *testing.T) {
	h := newHarness(t, acmecorpMap(t), html(
...
func TestResidualEnvironmentURL(t *testing.T) {
	h := newHarness(t, acmecorpMap(t), html(`<img src="https://acmecorp.ddev.site/app/x.png">`))
	_, got := h.get(t, variantHost, "/")
...
func TestRequestBodyRewritten(t *testing.T) {
	for _, c := range []struct{ name, ctype, body, want string }{
		...
		t.Run(c.name, func(t *testing.T) {
			h := newHarness(t, acmecorpMap(t), html("ok"))
...
func TestOversizeBodyPassesThrough(t *testing.T) {
	...
	h := newHarness(t, acmecorpMap(t), html("ok"), func(p *Proxy) { p.MaxBody = 1024 })
...
func TestSelfRedirectGuard(t *testing.T) {
	...
	uploads := func(w http.ResponseWriter, r *http.Request) {
		// Exactly what `rewrite ^ https://www.acmecorp.fi$request_uri redirect`
		// emits for a file that is not on disk.
		http.Redirect(w, r, canonical+r.URL.Path, http.StatusFound)
	}

	t.Run("passes through, counted", func(t *testing.T) {
		h := newHarness(t, acmecorpMap(t), uploads)
	...
	t.Run("strict-origins returns 404 instead", func(t *testing.T) {
		h := newHarness(t, acmecorpMap(t), uploads, func(p *Proxy) { p.StrictOrigins = true })
...
func TestSetCookieDomainDropped(t *testing.T) {
	h := newHarness(t, acmecorpMap(t), func(w http.ResponseWriter, r *http.Request) {
		w.Header().Add("Set-Cookie", "wordpress_logged_in=abc; Domain=.www.acmecorp.fi; Path=/; HttpOnly")
...
func TestRequestDirectionMechanics(t *testing.T) {
	...
	if h.seen.Host != "www.acmecorp.fi" {
...
func TestUpstreamFailureSurfaced(t *testing.T) {
	...
	p := &Proxy{Upstream: target, Map: acmecorpMap(t), Stats: rewrite.NewStats(false)}
	front := httptest.NewServer(p.Handler())
	defer front.Close()
