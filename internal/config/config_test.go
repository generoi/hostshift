...
// pellervoYAML is the real five-blog map, taken from the repo's robo.yml. The
...
sites:
  - name: main
    canonical: https://pellervo.pellervo.genero-dev.com
    base:      https://pellervo.ddev.site
  - name: kodin
    canonical: https://kodinpellervo.fi
    base:      https://kodinpellervo.ddev.site
  - name: maatilan
    canonical: https://maatilanpellervo.fi
    base:      https://maatilanpellervo.ddev.site
  - name: omapiha
    canonical: https://omapiha.info
    base:      https://omapiha.ddev.site
  - name: otlehti
    canonical: https://otlehti.pellervo.genero-dev.com
...
func TestFiveBlogSite(t *testing.T) {
	dir := t.TempDir()
	write(t, dir, "hostshift.yaml", pellervoYAML)
	...
	want := map[string]string{
		"main":     "https://wt-a--pellervo.ddev.site",
		"kodin":    "https://wt-a--kodinpellervo.ddev.site",
		"maatilan": "https://wt-a--maatilanpellervo.ddev.site",
...
func TestDDEVDefaults(t *testing.T) {
	dir := t.TempDir()
	// Verbatim from herrfors/.ddev/config.yaml: the additional hostname is
	// "nat.herrfors", not "nat", so the local hosts come out fully qualified.
	...
	want := [][2]string{
		{"https://herrfors.ddev.site", "https://wt-a--herrfors.ddev.site"},
		{"https://nat.herrfors.ddev.site", "https://wt-a--nat.herrfors.ddev.site"},
	}
...
`)
	...
	if len(res.Map.Sites) != 1 || res.Map.Sites[0].Canonical.Host != "www.herrfors.fi" {
...
version: 1
sites:
  - {name: main, canonical: https://herrfors.ddev.site}
  - {name: nat,  canonical: https://nat.herrfors.ddev.site}
`)
	res, err := Load(dir, Flags{Slug: "wt-a"})
	if err != nil {
...
func TestDDEVConfigOverridesAreMerged(t *testing.T) {
	...
	if name != "herrfors-wt-a" {
		t.Errorf("name = %q; DDEV would say herrfors-wt-a", name)
	}
	if tld != "ddev.site" {
		t.Errorf("tld = %q", tld)
	}
	...
	if got := res.Map.Sites[0].Canonical.Host; got != "herrfors-wt-a.ddev.site" {
