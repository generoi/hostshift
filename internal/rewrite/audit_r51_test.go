...
func TestR51TheSchemeArmInventsPercentEncodingItDidNotFind(t *testing.T) {
	...
	if dec := string(stripForCSS([]byte(out), ctlJoin).b); !strings.Contains(dec, variant) {
	...
	if dec := string(stripForRefs([]byte(rout), ctlJoin).b); !strings.Contains(dec, variant) {
...
func TestR51TheOctalDecoderKeepsItsRange(t *testing.T) {
	...
	for _, c := range stripForJSONEscCtl([]byte(tab), ctlJoin).b {
		if c < 0x20 || c == 0x7F {
