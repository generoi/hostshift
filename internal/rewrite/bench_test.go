...
func BenchmarkIdentity(b *testing.B) {
	in := corpusBytes(b)
	m := benchMatcher(b, "https://www.herrfors.fi", "https://www.herrfors.fi")
	b.SetBytes(int64(len(in)))
	b.ReportAllocs()
	for b.Loop() {
		if _, err := io.Copy(io.Discard, NewHTML(bytes.NewReader(in), m, nil, Options{})); err != nil {
...
func BenchmarkRewrite(b *testing.B) {
	in := corpusBytes(b)
	m := benchMatcher(b, "https://www.herrfors.fi", "https://wt-a--herrfors.ddev.site")
	b.SetBytes(int64(len(in)))
	b.ReportAllocs()
	for b.Loop() {
		if _, err := io.Copy(io.Discard, NewHTML(bytes.NewReader(in), m, nil, Options{})); err != nil {
...
func BenchmarkMatcherValue(b *testing.B) {
	m := benchMatcher(b, "https://www.herrfors.fi", "https://wt-a--herrfors.ddev.site")
	hit := []byte("https://www.herrfors.fi/app/uploads/2025/07/image-1024x768.jpg")
