...
func TestEveryAttributeScan(t *testing.T) {
	...
	cases := []struct{ name, in, want string }{
		...
		{
			// M0 found five of these in herrfors' production database. A browser
...
// production.
//
// It runs over the whole corpus with the real herrfors canonical set.
func TestNoDereferenceableProductionOrigin(t *testing.T) {
	canonical := []string{
		"https://www.herrfors.fi",
		"https://www.herrforsnat.fi",
...
func TestForeignContentGapQuantified(t *testing.T) {
	m := pairMatcher(t, "https://www.herrfors.fi", "https://v.example")
