...
// makes the leftmost-label variant scheme safe: "herrfors.ddev.site" is
// canonical and "wt-a--herrfors.ddev.site" contains it, but "//herrfors.ddev.site"
// does not occur inside "//wt-a--herrfors.ddev.site", so a second pass is a
...
func (m *Matcher) rewrite(b []byte, limit int, surface string, explain bool) ([]byte, int, []Event) {
	...
	for match := iter.Next(); match != nil; match = iter.Next() {
		...
		if end < len(b) && b[end] == '.' && delimAt(b, end+1) {
			end++
		}
