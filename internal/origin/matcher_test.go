...
//
//	https://herrfors.ddev.site:8081:8081/wp/wp-includes/…
//
...
func TestAnchoredOriginsNotBareHosts(t *testing.T) {
	m := mk(t, "https://herrfors.ddev.site", "https://wt-a--herrfors.ddev.site")
	...
	cases := []struct{ in, want string }{
		...
		{"https://wt-a--herrfors.ddev.site/x", "https://wt-a--herrfors.ddev.site/x"},
		{"//wt-a--herrfors.ddev.site/x", "//wt-a--herrfors.ddev.site/x"},
		// A longer host that merely starts with the canonical one.
		{"https://herrfors.ddev.site.evil.example/x", "https://herrfors.ddev.site.evil.example/x"},
		// A subdomain of the canonical host.
		{"https://a.herrfors.ddev.site/x", "https://a.herrfors.ddev.site/x"},
		// Bare hostname in prose is explicitly out of scope (test 28).
		{"visit herrfors.ddev.site for details", "visit herrfors.ddev.site for details"},
		// The real thing.
		{"https://herrfors.ddev.site/x", "https://wt-a--herrfors.ddev.site/x"},
	}
	for _, c := range cases {
...
func TestIdempotent(t *testing.T) {
	for _, c := range []struct{ from, to, in string }{
		{"https://herrfors.ddev.site", "https://wt-a--herrfors.ddev.site", "https://herrfors.ddev.site/a"},
		{"https://www.herrfors.fi", "https://wt-a--herrfors.ddev.site", "https://www.herrfors.fi/a"},
		// The port-only variant that produced the double-port bug.
		{"https://herrfors.ddev.site", "https://herrfors.ddev.site:8081", "https://herrfors.ddev.site/a"},
	} {
		m := mk(t, c.from, c.to)
		once := rw(t, m, c.in)
		twice := rw(t, m, once)
...
// percent-encoded occurrences in herrfors' production database, and the
// JSON-escaped form is what the REST API emits at render time.
func TestEncodedForms(t *testing.T) {
	m := mk(t, "https://www.herrfors.fi", "https://v.example")
	cases := []struct{ in, want string }{
		{`https://www.herrfors.fi/a`, `https://v.example/a`},
		{`http://www.herrfors.fi/a`, `https://v.example/a`},
		{`//www.herrfors.fi/a`, `//v.example/a`},
		{`https:\/\/www.herrfors.fi\/a`, `https:\/\/v.example\/a`},
		{`\/\/www.herrfors.fi\/a`, `\/\/v.example\/a`},
		{`redirect_to=https%3A%2F%2Fwww.herrfors.fi%2Fwp-admin%2F`, `redirect_to=https%3A%2F%2Fv.example%2Fwp-admin%2F`},
		{`%2F%2Fwww.herrfors.fi%2Fa`, `%2F%2Fv.example%2Fa`},
		// Case-insensitive on scheme and host (PLAN §5.5).
		{`HTTPS://WWW.HERRFORS.FI/a`, `https://v.example/a`},
		// Explicit default port compares equal to the bare host, and serialises
		// without it.
		{`https://www.herrfors.fi:443/a`, `https://v.example/a`},
		// A non-default port is a *different* origin and must not match.
		{`https://www.herrfors.fi:8080/a`, `https://www.herrfors.fi:8080/a`},
	}
	for _, c := range cases {
		if got := rw(t, m, c.in); got != c.want {
			t.Errorf("Rewrite(%q)\n got %q\nwant %q", c.in, got, c.want)
...
func TestMultisiteCrossBlog(t *testing.T) {
	m, err := NewMatcher([]Pair{
		...
		{Name: "nat", Canonical: MustParse("https://www.herrforsnat.fi"), Variant: MustParse("https://nat.wt-a--herrfors.ddev.site")},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	in := `<a href="https://www.herrforsnat.fi/x">on <a href="https://www.herrfors.fi/y">`
	want := `<a href="https://nat.wt-a--herrfors.ddev.site/x">on <a href="https://wt-a--herrfors.ddev.site/y">`
...
func TestValidateRejectsBadMaps(t *testing.T) {
	cases := []struct {
		...
		{"variant is matched by a canonical", []Pair{
			...
			// nat.herrfors.ddev.site -> nat.wt-a.herrfors.ddev.site would be
...
func TestValidateAcceptsContainment(t *testing.T) {
	m, err := NewMatcher([]Pair{
		{Name: "main", Canonical: MustParse("https://herrfors.ddev.site"), Variant: MustParse("https://wt-a--herrfors.ddev.site")},
		{Name: "nat", Canonical: MustParse("https://nat.herrfors.ddev.site"), Variant: MustParse("https://nat.wt-a--herrfors.ddev.site")},
	})
	if err != nil {
		t.Fatal(err)
	}
