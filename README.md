...
Only the variant is listed. `acme-wt-a.ddev.site` is the project's own hostname
and DDEV gives it that from `name` on its own; anything in the tracked
`.ddev/config.yaml` is inherited too, because DDEV *appends* override lists
...
Both variants, and nothing else — `acme-wt-a` and `shop.acme` are already there,
one from `name` and one from the tracked `.ddev/config.yaml`.
