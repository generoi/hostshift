...
`)
	...
	if got["url"] != "https://herrfors.ddev.site" {
...
func TestWPCLIWarnsAboutStaleAliases(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, dir, ".ddev/config.yaml", "name: herrfors\n")
...
func TestCheckExitCodes(t *testing.T) {
	for _, c := range []struct {
		...
		{"duplicate canonical", "version: 1\nsites:\n" +
			"  - {name: a, canonical: https://www.herrfors.fi, base: https://a.herrfors.ddev.site}\n" +
			"  - {name: b, canonical: https://www.herrfors.fi, base: https://b.herrfors.ddev.site}\n", exitConfig},
		{"not yaml", "sites: [\n", exitConfig},
	} {
...
func TestMapEnv(t *testing.T) {
	...
	if env["HOSTSHIFT_VARIANTS"] != "wt-a--herrfors.ddev.site,wt-a--nat.herrfors.ddev.site" {
	...
	if env["HOSTSHIFT_WEB_HOSTS"] != "herrfors.ddev.site,nat.herrfors.ddev.site" {
	...
	if !strings.Contains(errOut, "- wt-a--herrfors\n") {
...
func TestRewriteFilter(t *testing.T) {
	dir := ddevProject(t, "")
	in := `<a href="https://herrfors.ddev.site/x">k</a>`
	...
	if out != `<a href="https://wt-a--herrfors.ddev.site/x">k</a>` {
		t.Errorf("got %s", out)
	}

	t.Run("reverse maps the request direction", func(t *testing.T) {
		code, out, _ := run(t, `<a href="https://wt-a--herrfors.ddev.site/x">k</a>`,
			cmdRewrite, "-C", dir, "--slug", "wt-a", "--reverse", "--quiet")
		if code != exitOK {
			t.Fatalf("exit %d", code)
		}
		if out != `<a href="https://herrfors.ddev.site/x">k</a>` {
			t.Errorf("got %s", out)
		}
	})
	...
	t.Run("a type outside the rewritable set is passed through", func(t *testing.T) {
		css := "body{background:url(https://herrfors.ddev.site/a.png)}"
	...
	t.Run("json is rewritten and stays valid", func(t *testing.T) {
		_, out, _ := run(t, `{"link":"https:\/\/herrfors.ddev.site\/x"}`, cmdRewrite,
			"-C", dir, "--slug", "wt-a", "--type", "application/json", "--quiet")
		if out != `{"link":"https:\/\/wt-a--herrfors.ddev.site\/x"}` {
			t.Errorf("got %s", out)
		}
	})
}
